...
if __name__ == '__main__':
    for fun_id in [1,8,14,19,25,32,38,44,50,56,62]:
        MaxFEs = 4E3
        timestamp = time.strftime("%Y-%m-%d_%H-%M-%S", time.localtime())

        logs_dir = f'repository/logs/baseline/lcc_cmaes/brax/maxfes_{MaxFEs:.0E}'.replace('+0', '') + f'/f{fun_id}/{timestamp}'
        output_path = f'repository/data/baseline/lcc_cmaes/brax/maxfes_{MaxFEs:.0E}'.replace('+0', '') + f'/f{fun_id}/{timestamp}'

        fitness_record, average_time = run_parallel_task(nda_optimization_task, parallel_num=5, fun_id=fun_id, MaxFEs=MaxFEs)
