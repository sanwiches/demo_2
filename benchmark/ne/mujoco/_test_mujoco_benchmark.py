"""
MuJoCo基准测试模块的测试脚本 / Test script for MuJoCo Benchmark module.

此脚本演示如何使用MuJoCo Benchmark类 / This script demonstrates how to use the MuJoCo Benchmark class
该类与cec2013lsgo具有相同的接口 / with the same interface as cec2013lsgo.

使用方法 / Usage:
    python _test_mujoco_benchmark.py                    # 自动检测设备 / Auto-detect device
    python _test_mujoco_benchmark.py --device cuda      # 指定CUDA设备 / Specify CUDA device
    python _test_mujoco_benchmark.py --device cpu       # 指定CPU设备 / Specify CPU device
"""
...
# 必须在导入 jax 之前导入 CUDA 插件 / Must import CUDA plugin before jax
try:
...
import numpy as np
import jax
from .mujoco_benchmarks import Benchmark
...
DEFAULT_MAX_FUNCTIONS_DISPLAY = 60
...
def print_device_info(device: str) -> None:
    ...
    logger.info(f"MuJoCo Benchmark测试/MuJoCo Benchmark Test (设备/Device: {device})")
    logger.info(separator)
    logger.info(f"JAX版本/JAX Version: {jax.__version__}")
    logger.info(f"JAX可用设备/JAX Available devices: {jax.devices()}")
    logger.info(f"默认后端/Default backend: {jax.default_backend()}")


def print_available_functions(benchmark: Benchmark,
                              max_display: int = DEFAULT_MAX_FUNCTIONS_DISPLAY,
                              full_mode: bool = False) -> None:
    """
    ...
    Args:
        ...
        full_mode: 是否完整显示（获取所有函数真实维度）/ Whether to show full list with real dimensions
    """
    ...
    # 显示函数列表 / Display function list
    if full_mode:
        logger.info("完整模式/Full mode: 正在逐个获取所有函数维度（可能较慢）...")
        display_count = len(functions)
        for func in functions:
            try:
                info = benchmark.get_info(func['id'])
                dim = info['dimension']
            except Exception as e:
                dim = f"ERROR: {e}"
            logger.info(
                f"  ID {func['id']:2d}: {func['env_name']:30s} "
                f"类别/category={func['category']:15s} 维度/dim={dim}"
            )
    else:
        display_count = min(max_display, len(functions))
        for func in functions[:display_count]:
            logger.info(
                f"  ID {func['id']:2d}: {func['env_name']:30s} "
                f"类别/category={func['category']:15s} 维度/dim={func['dimension']}"
            )
...
def test_function_info(benchmark: Benchmark, func_id: int) -> None:
    ...
    if 'env_name' in info:
        ...
        logger.info(f"  Episodes数量/Num episodes: {info['num_episodes']}")
        logger.info(f"  最大步数/Max episode length: {info['max_episode_length']}")
    if 'best' in info:
        logger.info(f"  最优值/Best value: {info['best']}")
...
def test_mujoco_benchmark(device: Optional[str] = None,
                          full_mode: bool = False) -> None:
    """
    测试MuJoCo Benchmark类 / Test the MuJoCo Benchmark class.
    ...
    Args:
        ...
        full_mode: 是否完整显示所有函数维度 / Whether to show all function dimensions
    """
    # 检测CUDA可用性 / Check CUDA availability
    # jax.devices('cuda') 在未安装 GPU backend 时会直接抛错，而不是返回空列表。
    if device is None:
        try:
            device = "cuda" if len(jax.devices("cuda")) > 0 else "cpu"
        except RuntimeError:
            device = "cpu"
    ...
    print_available_functions(benchmark, full_mode=full_mode)

    if full_mode:
        logger.info("完整模式已完成维度统计，跳过后续示例评估。/ Full mode finished dimension listing, skip demo evaluations.")
        separator = "\n" + "=" * 50
        logger.info(separator)
    ...
    # 测试函数1: AcrobotSwingup / Test function 1: AcrobotSwingup
    test_function_info(benchmark, 1)
    test_function_retrieval(benchmark, 1)
    test_function_evaluation(benchmark, 1)

    # 测试函数10: SwimmerSwimmer6 / Test function 10: SwimmerSwimmer6
    test_function_info(benchmark, 10)
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description='测试MuJoCo基准 / Test MuJoCo Benchmark',
...
示例/Examples:
  ...
  %(prog)s --device cpu       使用CPU设备 / Use CPU device
    %(prog)s --full-mode        完整模式（显示全部函数真实维度）/ Full mode (show all real dimensions)
        """
    )
    ...
    parser.add_argument(
        '--full-mode',
        action='store_true',
        help='完整模式：逐个调用get_info显示全部函数真实维度 / Full mode: call get_info for all functions to show real dimensions'
    )
...
def main() -> None:
    ...
    test_mujoco_benchmark(device=args.device, full_mode=args.full_mode)
